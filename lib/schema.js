...
function createItemGetter(valueTest, emitter, parent, type) {
  ...
  return (target, key) => {
    ...
    if (typeof key === 'string' && value !== undefined) {
      ...
      if (factory) {
        return (cache
          || (cache = factory(value, emitter, path(parent, key))));
      }
    }
    return value;
  };
...
function createKeyValueItemWriter(valueTest) {
  return (value, emitter, parent) => {
    return new Proxy(value, {
...
function verifyWriter(value) {
  return value[RAW_SYMBOL] && JSON.parse(JSON.stringify(value))
    || failNotSchemaObject();
}
...
function mapVerifyer(keyVerify, valueTest) {
  ...
  return lazy(verify, 'write', () => createKeyValueItemWriter(valueTest));
}
