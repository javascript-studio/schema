...
function createPropertyGetter(tests, type) {
  return function (target, key) {
    ...
    if (typeof key === 'string') { // Key might be Symbol
      ...
      if (factory) {
        return factory(Reflect.get(target, key));
      }
    }
...
schema.opt = function (spec) {
  ...
  const opt = (value, property) => value === undefined || test(value, property);
...
schema.one = function () {
  ...
  const one = value => tests.some(test => test(value));
  return lazy(one, 'specName', () => `one(${specs.map(specName).join(', ')})`);
};

schema.object = function (key_spec, value_spec) {
  ...
  return lazy(object, 'specName',
    () => `object(${specName(key_spec)}, ${specName(value_spec)})`);
};
