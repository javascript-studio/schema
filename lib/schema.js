...
function createTypeError(value, name, property) {
  const actual = stringify(value);
  return new TypeError(property
    ? `Expected property "${property}" to be ${name} but got ${actual}`
    : `Expected ${name} but got ${actual}`);
}
...
function verifyer(fn, spec) {
  return (value, property) => fn(value, property) === false
    ? failSchemaValidation(createTypeError(value, specName(spec), property))
    : value;
}
...
function assertObject(spec) {
  if (!type.object(spec)) {
    throw createTypeError(spec, 'object');
  }
}
...
schema.array = (spec) => {
  const test = spec.verify ? spec : (assertObject(spec), object(spec));
