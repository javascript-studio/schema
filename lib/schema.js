...
function custom(fn, name) {
  return (value, property, op) => {
    if (fn(value, property, op) === false) {
      if (property) {
...
function createWriter(schemas, schema_keys) {
  ...
  const propertySetter = function (target, key, value) {
    getSchema(schemas, key)(value, key, 'set');
...
createSchema.opt = function (spec) {
  const schema = createSchema(spec);
  return (value, property, op) => {
    if (value !== undefined || op === 'set') {
      schema(value, property, op);
    }
  };
};
