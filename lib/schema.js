...
schema.object = (spec) => {
  const o = object(spec);
  return lazy(o, 'specName', () => `object(${specName(spec)})`);
};

schema.keyValue = function (key_spec, value_spec) {
  const keyTest = object(key_spec);
  const valueTest = object(value_spec);
