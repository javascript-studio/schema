...
function objectVerifyer(tests, spec_options = {}) {
  const verify = createVerify(tests);
  verify.verify = verifyWriter(verify, spec_options);
  lazy(verify, 'read', () => createReader(tests, verify, spec_options));
  lazy(verify, 'write', () => createWriter(tests, spec_options));
  return verify;
}
...
function verifyWriter(verify, spec_options) {
  return (value) => verify(raw(value), spec_options);
}
...
function createPropertySetter(tests, options, base) {
  return (target, key, value) => {
    if (typeof key === 'string') {
      value = unwrap(value);
      const path = objectPath(base, key);
      getTest(tests, key, base, options).verify(value, options, path, true);
      if (options.emitter) {
